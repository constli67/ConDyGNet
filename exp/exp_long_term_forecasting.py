...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_criterion(self):
        # 你的原版 exp 这里固定 MSE，用于验证/早停（我们保持不变）
    ...
    # =========================
    # 训练用：时域 + 频域 混合 MAE（像你给的那个人）
    # loss = (1-a)*MAE_time + a*MAE_freq
    #
    # 优先使用 args.loss_alpha；若不存在则回退 args.alpha
    # =========================
    def time_freq_mae(self, batch_y, outputs):
        # batch_y, outputs: [B, T, C]
        if hasattr(self.args, "loss_alpha"):
            a = float(getattr(self.args, "loss_alpha"))
        else:
            a = float(getattr(self.args, "alpha", 0.0))
        ...
        # freq mae
        out_f = torch.fft.rfft(outputs.float(), dim=1)
        y_f = torch.fft.rfft(batch_y.float(), dim=1)
        f_loss = (out_f - y_f).abs().mean()

        return (1.0 - a) * t_loss + a * f_loss

    def vali(self, vali_data, vali_loader, criterion):
        # ✅ 保持你原来的验证逻辑：只用 criterion（MSE），不引入频域
        total_loss = []
        self.model.eval()
        with torch.no_grad():
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        outputs, _ = self.model(batch_x, self.masks, is_training=True)

                        f_dim = -1 if self.args.features == 'MS' else 0
                        outputs = outputs[:, -self.args.pred_len:, f_dim:]
                        batch_y_cut = batch_y[:, -self.args.pred_len:, f_dim:]

                        # ✅ 训练时改成时域+频域混合 loss；彻底去掉 moe_loss
                        loss = self.time_freq_mae(batch_y_cut, outputs)
                else:
                    outputs, _ = self.model(batch_x, self.masks, is_training=True)
